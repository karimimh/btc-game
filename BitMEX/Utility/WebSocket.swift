    ...
    func inflate(_ bufin : UnsafePointer<UInt8>, length : Int, final : Bool) throws -> (p : UnsafeMutablePointer<UInt8>, n : Int){
        ...
        for i in 0 ..< 2{
            if i == 0 {
                ...
                strm.next_in = inflateEnd.withUnsafeBufferPointer { (buffer) -> UnsafePointer<UInt8>? in
...
private class InnerWebSocket: Hashable {
    ...
    func openConn() throws {
        ...
        let up: UnsafePointer<UInt32> = keyb.withUnsafeBufferPointer { (buffer) -> UnsafePointer<UInt32> in
            return buffer.baseAddress!
        }
        let rkey = Data(bytes: up, count: 16).base64EncodedString(options: NSData.Base64EncodingOptions(rawValue: 0))
            ...
            if i == 0  {
                ...
                if line.hasPrefix("\t") || line.hasPrefix(" ") {
                    ...
                    if let r = line.range(of: ":") {
                        key = trim(String(line[line.startIndex..<r.lowerBound]))
                        value = trim(String(line[r.upperBound..<line.endIndex]))
                    }
                }
